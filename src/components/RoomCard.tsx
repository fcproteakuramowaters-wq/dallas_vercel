...
export default function RoomCard({ id, title, price, image, slideImages, color = "blue", description = "", index = 0, onViewDetails, }: RoomCardProps) {
  const [currentSlide, setCurrentSlide] = useState(0);
  const images = slideImages && slideImages.length > 0 ? slideImages : [image];
  ...
  useEffect(() => {
    ...
    // Add staggered delay - each card starts sliding after index * 1200ms
    ...
    <div className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-2xl transition-shadow duration-300 flex flex-col">
      <div className="relative h-64 bg-gray-200 overflow-hidden group">
        <Image 
          ...
          alt={title} 
          fill 
          className="object-cover room-img w-full h-full"
          priority
        />
